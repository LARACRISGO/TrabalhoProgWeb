...
controller.listar = async (req, res) => {
    ...
    if(Object.keys(req.query).length > 0) { // Se houver query string
        ...
        try {
            ...
            const lista = await Idoso.find().populate('responsavel').populate('cuidador')
...
controller.obterUm = async (req, res) => {
    try {
        ...
        const obj = await Idoso.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        const obj = await Idoso.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        const id = req.body._id
        const obj = await Idoso.findByIdAndDelete(id)
        if(obj){
            res.status(204).end()
        }
        else {
...
async function busca(req, res) {
    ...
    try {
        const lista = await Idoso.find(criterio);
        res.send(lista)
    }
    catch(erro) {
        console.log(erro)
