...
controller.obterUm = async (req, res) => {
    try {
        ...
        const obj = await Medicamento.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        const obj = await Medicamento.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        const id = req.body._id
        const obj = await Medicamento.findByIdAndDelete(id)
        if(obj){
            res.status(204).end()
        }
        else {
...
async function busca(req, res) {
    ...
    try {
        const lista = await Medicamento.find(criterio);
        res.send(lista)
    }
    catch(erro) {
        console.log(erro)
